...
alter table records add constraint cons_spotId foreign key(spot_id) references spots(spot_id) on update cascade on delete cascade;		-- 使用alter语句增加外键
desc records;
...
-- 插入管理员
insert into users(username, password, role) values("zhangsan", "123456", 0);

-- 插入景点数据
insert into spots(spot_name, location, history) values
...
-- 插入学习记录
insert into records(user_id, spot_id, produce_time, learn_note) values(2, 1, "2025/6/23", "好好学习，天天向上");
select * from records;

-- 插入公告
insert into announcements(title, content, post_time) values("升级维护通知", "为提升系统性能、优化用户体验，并确保数据安全，[系统名称/平台名称]将于近期进行计划内升级维护。
